...
OakArray.prototype.__set = function __set (key, value) {
  ...
  var node = descend(data, key, new_depth)

  node[key & KEY_MASK] = value
...
OakArray.prototype.delete = function delete_ (key) {
  var data = shallow_clone(this.data)
  var node = descend(data, key, this.depth)

  delete node[key & KEY_MASK]

  return new OakArray(data, this.depth, this.length)
}
...
function descend (node, key, depth) {
  for (var shift = (depth - 1) * KEY_BITS;
       shift >= 5;
       shift -= KEY_BITS) {
