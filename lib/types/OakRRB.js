...
OakArray.prototype.concat = function concat (other) {
  // We gonna grow to at least two levels because the balancing
  // algorithm requires at least two. We're shrinking it later
...
function balance (back_depth, left, right, middle_left, middle_right, new_left, new_right) {
  ...
  while (i < total_length) {
    ...
    // Add full nodes as-is
    while (i < total_length && node.length - offset >= MIN_ELEMENTS) {
      push(node)
      node = get()
    }
    ...
    while (i < total_length && node.length - offset < MIN_ELEMENTS) {
      ...
      // Find next non-empty node to steal element from to balance
      // this one
      do {
        next = get()
    ...
    // Here the node is as good as it ever be so we commit it
    push(node)
    node = next
  }
...
;(function test () {
  ...
  var c = a.concat(new OakArray([[15], a.data], 4, a.length)).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a).concat(a);

  //console.log(a.concat(a).data)
  ...
  console.log(new OakArray([[], [1,2,3,4], [5,6], [], [7]], 2, 7)
              .concat(new OakArray([8,9], 1, 2)))
})();
