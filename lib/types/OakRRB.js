...
function OakArray (data, depth, length, tail, data_length) {
  this.data = data || []
  this.tail = tail || new Array(ELEMENTS)
  this.depth = depth || 1
  this.length = length || 0
  this.data_length = data_length === undefined ? length || 0 : data_length
}
...
OakArray.prototype.__set = function __set (key, value) {
  var new_length = Math.max(this.length, key + 1)
  var tail_key = key - this.data_length

  if (key < this.data_length) {
    // Update in data
    ...
    var index = key
    while (--depth) {
      for (var i = index >> (depth * KEY_BITS); index >= node[0][i]; ++i);
    ...
    return new OakArray(data, depth, this.length, tail, this.data_length)
  } else if (tail_key >= 0 && tail_key < ELEMENTS) {
    // Update existing tail
    var tail = this.tail.slice()
    tail[tail_key] = value

    return new OakArray(this.data, this.depth, new_length, tail, this.data_length)
  } else if (this.data_length === 0 && tail_key < ELEMENTS * 2) {
    var new_tail = new Array(ELEMENTS)
    new_tail[tail_key - ELEMENTS] = value
    return new OakArray(this.tail, 1, new_length, new_tail, ELEMENTS)
  } else {
    var depth = this.depth
    var data = grow(this.data, this.depth, depth)
    var path = [data]
    var node = data
    ...
    // Find spot to insert the tail
    var child
    for (i = depth - 2; i >= 0; --i) {
      node = path[i]
      if (node.length - 1 < ELEMENTS) {
        node = node.slice()
        node[0] = []
        node.push(grow(this.tail, 1, depth - i - 1))
        put_ranges(node, depth - i === 2)
        child = node
        --i
        break
      }
    ...
    if (i >= 0) {
      // if loop was broken before i reached 0, means tail was
      // inserted and we don't have to grow the tree
      for (; i >= 0; --i) {
        node = path[i].slice()
        node[0] = []
      ...
      ++depth
      data = [[], data, grow(this.tail, 1, depth - 1)]
      put_ranges(data, depth === 2)
    }

    var tail = new Array(ELEMENTS)
    tail[0] = value
    //console.log(JSON.stringify(data))
    return new OakArray(data, depth, new_length, [value], key)

    //console.log(path)

    // var depth = this.depth
    // var data = this.data
    // var empty_slots = ELEMENTS - (depth > 1 ? data[0].length : 1)

    // while (this.data_length + (empty_slots << (KEY_BITS * (depth - 1))) < key) {
    //   ++depth
    // }

    // var old_tail = grow(this.tail, 1, depth - 1)

    // if (depth > this.depth) {
    //   var left = grow(data, this.depth, depth - 1)
    //   data = [[], left, old_tail]
    // } else {
    //   data = data.slice()
    //   data[0] = []
    ...
    // // Add just enough number of dummy nodes before the new tail
    // // begins
    // var jump = 1 << (KEY_BITS * (depth - 1))
    // for (var sum = this.data_length + ELEMENTS + jump; sum < key; sum = sum + jump) {
    //   data[0].push(sum)
    // }
    ...
    // return new OakArray(data, depth, key + 1, tail, key)
  }
}
...
function balance (left, right, middle_left, middle_right, new_left, new_right, leaf_distance) {
  ...
  var nodes_to_reduce =
      total_length
      - ALLOWED_IMBALANCE
      - Math.floor((count(left) + count(right) - (offset * 2)) / ELEMENTS)
